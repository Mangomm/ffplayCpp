...
void VideoState::set_default_window_size(int width, int height, AVRational sar){
    ...
    // 2. 通过用户指定的显示宽高，以及视频流实际的分辨率，得出最终渲染的宽高。
...
int VideoState::ReadThread(){
    ...
    */
    ...
    if (st_index[AVMEDIA_TYPE_SUBTITLE] >= 0) { // 如果有字幕流则打开字幕流
        ...
        //SPDWARN("AVMEDIA_TYPE_SUBTITLE not support");
    }
...
bool VideoState::Init(HWND handle, std::string url) {

    int flags;

    _filename = url;

    opt_add_vfilter(NULL, NULL, "subtitles=output.mkv:si=0");//给视频添加过滤器，用于显示字幕，可以看到即使字幕代码被注释掉，仍然可以
                            // 通过过滤器进行显示.
...
int VideoState::WriteThread(){
    ...
    for (;;) {
        ...
        case FF_QUIT_EVENT:				/* 自定义事件,在这用于被动退出 */
            ...
            //SPDERROR("write_thread recevice FF_QUIT_EVENT, url: {}", cur_stream->_filename.c_str());
            return -1;
        case FF_INITIATIVE_QUIT_EVENT:	/* 自定义事件,在这用于主动退出(用户主动调关闭) */
            //cur_stream->do_exit();
            //SPDERROR("write_thread recevice FF_INITIATIVE_QUIT_EVENT, url: {}", cur_stream->_filename.c_str());
...
int write_thread1(void *arg) {
    ...
    printf("WriteThread exit, errno: %d\n", ret);
    return 0;
