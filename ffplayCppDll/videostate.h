...
// 定义该宏表示使用dll进行播放视频流
#define USE_DLL_PLAY
int FFPLAYCPPDLLSHARED_EXPORT write_thread1(void *arg);
...
#define FRAME_QUEUE_SIZE FFMAX(SAMPLE_QUEUE_SIZE, FFMAX(VIDEO_PICTURE_QUEUE_SIZE, SUBPICTURE_QUEUE_SIZE))
    ...
    */
    typedef struct Frame {
        ...
        int		uploaded;           // 记录该帧是否已经显示。1=已经显示，0未显示
    ...
    */
    ...
    class FFPLAYCPPDLLSHARED_EXPORT VideoState
    {
    public:
        VideoState();
        ~VideoState();

        bool Init(HWND handle, Properties pp);// 初始化SDL相关以及其他变量
    ...
    private:
        ...
        int _width, _height, _xleft, _ytop;           // 宽、高，x起始坐标，y起始坐标。宽高的值可能是用户传进或者默认值。得到该宽高后，会最终通过calculate_display_rect
                                                      // 来算出在屏幕显示的宽高，即保存在calculate_display_rect函数的参数1。
        ...
        Properties _properties;                         // tyycode，用于初始化的属性变量
